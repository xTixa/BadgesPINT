...
export default function EmailSignatureManager({ Layout, layoutProps = {}, variant = "consultant" }) {
  const { t } = useTranslation();
  const isResponsible = variant === "responsible";
  const title = isResponsible ? t("consultor.emailSignature.responsibleTitle") : t("consultor.emailSignature.title");
  const subtitle = isResponsible ? t("consultor.emailSignature.responsibleSubtitle") : t("consultor.emailSignature.subtitle");
  const badgesInSignatureLabel = isResponsible ? t("consultor.emailSignature.responsibleBadgesInSignature") : t("consultor.emailSignature.badgesInSignature");
  const noBadgesLabel = isResponsible ? t("consultor.emailSignature.noBadgesResponsible") : t("consultor.emailSignature.noBadgesObtained");
  ...
  return (
    <Layout title={title} subtitle={subtitle} {...layoutProps}>
      {loading ? <EmptyState message={t("consultor.emailSignature.loading")} icon="bi-hourglass-split" /> : error && !signature ? <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-rose-700">{error}</div> : <>
        ...
        <div className="grid gap-6 xl:grid-cols-2"><section className="rounded-3xl bg-white p-6 shadow-sm"><label className="mb-6 flex items-center justify-between rounded-2xl border border-slate-200 p-4"><div><div className="font-bold text-slate-900">{t("consultor.emailSignature.enableSignature")}</div><p className="text-sm text-slate-500">{t("consultor.emailSignature.enableSignatureHint")}</p></div><input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} className="h-5 w-5 accent-blue-600" /></label><div className="mb-3 flex items-center justify-between"><h2 className="font-bold">{badgesInSignatureLabel}</h2><span className="text-xs text-slate-500">{selected.length}/6</span></div><div className="space-y-2">{signature.available_badges.map((badge) => <button type="button" key={badge.id} onClick={() => toggleBadge(Number(badge.id))} className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-left ${selected.includes(Number(badge.id)) ? "border-blue-500 bg-blue-50" : "border-slate-200"}`}>{badge.image_url ? <img src={badge.image_url} alt="" className="h-10 w-10 object-contain" /> : <i className="bi bi-patch-check-fill text-2xl text-blue-600" />}<span className="flex-1"><strong className="block text-sm">{badge.name}</strong><span className="text-xs text-slate-500">{badge.level}</span></span>{selected.includes(Number(badge.id)) && <i className="bi bi-check-circle-fill text-blue-600" />}</button>)}{!signature.available_badges.length && <EmptyState message={noBadgesLabel} icon="bi-award" />}</div><button onClick={save} disabled={saving} className="mt-5 w-full rounded-2xl bg-blue-600 px-5 py-3 font-semibold text-white disabled:opacity-60">{saving ? t("consultor.emailSignature.saving") : t("consultor.emailSignature.saveSignature")}</button></section>
