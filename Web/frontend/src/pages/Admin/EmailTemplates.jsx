...
export default function Configuracoes() {
  ...
  return <div className="admin-shell"><Sidebar user={{ role: "admin", name: "Admin" }} /><main className="admin-main"><div className="mx-auto max-w-7xl space-y-6">
    <section className="rounded-3xl bg-gradient-to-r from-[#0F62FE] to-[#00AEEF] p-8 text-white shadow-lg"><p className="text-sm text-white/80">{t("admin.emailTemplates.eyebrow")}</p><h1 className="mt-1 text-3xl font-bold">{t("admin.emailTemplates.title")}</h1><p className="mt-2 text-white/80">{t("admin.emailTemplates.subtitle")}</p></section>
    {message && <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-700">{message}</div>}
    {loading ? <EmptyState message={t("admin.emailTemplates.loading")} icon="bi-hourglass-split" /> : <div className="grid gap-5 lg:grid-cols-12"><aside className="space-y-2 lg:col-span-3">{templates.map((item) => <button key={item.key} onClick={() => setSelectedKey(item.key)} className={`w-full rounded-2xl border p-4 text-left ${item.key === selectedKey ? "border-blue-500 bg-blue-50" : "border-slate-200 bg-white"}`}><strong className="block text-sm text-slate-900">{item.name}</strong><span className={`text-xs ${item.customized ? "text-emerald-600" : "text-slate-500"}`}>{item.customized ? t("admin.emailTemplates.customized") : t("admin.emailTemplates.defaultLabel")}</span></button>)}</aside>
      {selected && <div className="space-y-5 lg:col-span-9"><section className="rounded-3xl bg-white p-6 shadow-sm"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-xl font-bold">{selected.name}</h2><p className="text-sm text-slate-500">{t("admin.emailTemplates.variables")}: {selected.variables.map((item) => `{{${item}}}`).join(", ")}</p></div><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={draft.enabled} onChange={(e) => setDraft({ ...draft, enabled: e.target.checked })} className="accent-blue-600" />{t("admin.emailTemplates.active")}</label></div><label className="mb-2 block text-sm font-semibold">{t("admin.emailTemplates.subject")}</label><input value={draft.subject} onChange={(e) => setDraft({ ...draft, subject: e.target.value })} className="ui-input mb-5 w-full" /><label className="mb-2 block text-sm font-semibold">{t("admin.emailTemplates.htmlContent")}</label><textarea rows="12" value={draft.html_body} onChange={(e) => setDraft({ ...draft, html_body: e.target.value })} className="ui-input w-full font-mono text-sm" /><label className="mb-2 mt-5 block text-sm font-semibold">{t("admin.emailTemplates.plainTextVersion")}</label><textarea rows="4" value={draft.text_body} onChange={(e) => setDraft({ ...draft, text_body: e.target.value })} className="ui-input w-full" /><div className="mt-5 flex gap-2"><button onClick={save} disabled={saving} className="rounded-xl bg-blue-600 px-5 py-2.5 font-semibold text-white disabled:opacity-60">{saving ? t("admin.emailTemplates.saving") : t("admin.emailTemplates.saveTemplate")}</button><button onClick={reset} className="rounded-xl border border-slate-300 px-5 py-2.5 font-semibold text-slate-700">{t("admin.emailTemplates.resetDefault")}</button></div></section><section className="rounded-3xl bg-white p-6 shadow-sm"><h2 className="mb-2 font-bold">{t("admin.emailTemplates.preview")}</h2><p className="mb-4 text-sm text-slate-500"><strong>{t("admin.emailTemplates.subject")}:</strong> {draft.subject.replace(/{{\s*([a-z0-9_]+)\s*}}/gi, (_, key) => samples[key] || `{{${key}}}`)}</p><div className="rounded-2xl border border-slate-200 p-5" dangerouslySetInnerHTML={{ __html: preview }} /></section></div>}
    </div>}
  </div></main></div>;
}
