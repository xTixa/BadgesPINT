...
export default function Sidebar({ user }) {
  ...
  const navItemClass = ({ isActive }) =>
    ...
    } ${isActive ? "bg-[#2AA4BF] text-white" : "text-[#0D0D0D] hover:bg-[#2AA4BF]/10"}`;
  ...
  return (
    <>
      {isMobile && (
        <button
          type="button"
          className="fixed left-3 top-[74px] z-[1200] inline-flex h-[44px] w-[44px] items-center justify-center rounded-xl border border-[#013440] bg-[#2AA4BF] text-[#013440] lg:hidden"
      ...
      {isMobile && mobileOpen && (
        <div
          className="fixed inset-0 z-[1090] bg-[#0D0D0D]/40 lg:hidden"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#2AA4BF]/20">
          <div className={`flex items-center ${collapsed && !isMobile ? "justify-center w-full" : "gap-3"}`}>
            {(!collapsed || isMobile) && (
              <div>
                <span className="block text-[0.65rem] uppercase tracking-widest text-[#2AA4BF]">Painel</span>
                <span className="text-sm font-semibold text-[#0D0D0D]">{menu.title}</span>
              </div>
            )}
          </div>
          ...
          <button
            ...
            className={`rounded-lg border border-[#013440] bg-[#2AA4BF] p-1.5 text-[#013440] hover:bg-[#04C4D9] ${
        ...
        {(!collapsed || isMobile) && (
          <div className="px-4 py-3">
            <div className="rounded-lg bg-[#2AA4BF]/10 px-3 py-2">
              <div className="text-sm font-medium text-[#0D0D0D]">{user.name}</div>
            </div>
          </div>
        )}
        ...
        <nav className="flex-1 overflow-y-auto p-2">
          {(!collapsed || isMobile) && (
            <p className="px-2 pb-1 text-[0.65rem] uppercase tracking-[0.12em] text-[#2AA4BF]">Navegação</p>
          )}
        ...
        {menu.footer.length > 0 && (
          <div className="border-t border-[#2AA4BF] p-2">
            {(!collapsed || isMobile) && (
              <p className="px-2 pb-1 text-[0.65rem] uppercase tracking-[0.12em] text-[#2AA4BF]">Sistema</p>
            )}
            <ul className="m-0 list-none p-0">
